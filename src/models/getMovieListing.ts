...
export const getMovieListing = (page: number, size: number) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const count =
        (await prismaClient.$queryRaw`select count(*) as count from Solicitud s join Pelicula P on s.id = P.solicitud_id where estado = 'APROBADA' and aws_url is not null  limit ${offset},${size}`) as [
          { count: string }
        ];
